...
def classify_publication(receipt: dict | None, receipt_update: dict | None, exit_code: int, artifact_exists: bool, warnings: tuple[str, ...] = ()) -> PublicationOutcome:
    ...
    if not committed:
        diagnostics.append("receipt contradicts historical artifact commitment")
        return PublicationOutcome(PublicationState.COMMITTED_EVIDENCE_CONTRADICTION, True, tuple(diagnostics))
    if receipt.get("handoff_allowed") is not True:
        if exit_code != 2:
            diagnostics.append("handoff-blocked receipt has unexpected exit code")
            return PublicationOutcome(PublicationState.COMMITTED_EVIDENCE_CONTRADICTION, True, tuple(diagnostics))
    ...
    if exit_code != 0:
        diagnostics.append(f"accepted receipt has unexpected exit code {exit_code}")
        return PublicationOutcome(PublicationState.COMMITTED_EVIDENCE_CONTRADICTION, True, tuple(diagnostics))
