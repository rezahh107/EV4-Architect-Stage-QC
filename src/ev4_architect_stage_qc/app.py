...
class Application:
 ...
 def _build(self):
  ...
  self._row(f,8,'Project Gate Export Stage JSON',self.terminal,self.select_terminal,'Select Final Stage JSON'); self.final=ttk.Button(f,text='Validate and Publish Final Output',command=self.final_validation);self.final.grid(column=0,row=10,sticky='w')
 ...
 def _start(self,fn,args,status='ℹ Running validation'):
  if self.active:return
  self.active=True;self.pref.configure(state='disabled');self.final.configure(state='disabled');self.status.set(status); self.detail.set('The operation is running; please wait.')
