...
class Application:
 def __init__(self,root):
  ...
  root.title('EV4 Architect Stage QC'); root.minsize(720,480); apply(root); self._build(); root.protocol('WM_DELETE_WINDOW',self._close); root.after(100,self._poll); self._restore()
 def _build(self):
  ...
  self._row(f,8,'Project Gate Export Request JSON',self.terminal,self.select_terminal,'Select Export Request JSON'); self.final=ttk.Button(f,text='Run Final Validation',command=self.final_validation);self.final.grid(column=0,row=10,sticky='w')
