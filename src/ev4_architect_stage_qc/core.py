...
def run_final_publication(stage_folder:Path, terminal_path:Path, architect_root:Path, publication_root:Path|None=None):
 ...
 try:
  ...
  if result.published and result.location:
   committed_state='FINAL_COMMITTED_HANDOFF_BLOCKED' if result.receipt and result.receipt.get('handoff_allowed') is False else ('FINAL_PUBLISHED_COPY_WARNING' if not result.success else 'FINAL_PUBLISHED')
   try: write_json(generated/'architect-publication-summary.json',{'status':committed_state,'run_id':terminal['run_id'],'architect_repository':'rezahh107/EV4-Architect-Repo','architect_commit_sha':conn.commit,'publisher_branch':result.location.publisher_branch,'publisher_worktree':str(result.location.publisher_worktree),'official_artifact_path':str(result.location.artifact_path),'historical_receipt':result.receipt,'receipt_update':result.receipt_update,'authority_compatibility_mode':conn.compatibility_mode,'authority_files_verified':True})
